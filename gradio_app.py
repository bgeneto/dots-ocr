...
- Session ID: {pdf_result['session_id']}
        ...
        current_page_layout_image = (
            preview_image  # Use the original preview image by default
        )
            ...
            if current_result["cells_data"]:
                try:
                    current_page_json = json.dumps(
                        current_result["cells_data"], ensure_ascii=False, indent=2
                    )
                except:
        ...
        if pdf_result["temp_dir"]:
            download_zip_path = os.path.join(
                pdf_result["temp_dir"], f"layout_results_{pdf_result['session_id']}.zip"
            )
            try:
                with zipfile.ZipFile(
                    download_zip_path, "w", zipfile.ZIP_DEFLATED
        ...
        return (
            ...
            info_text,
            combined_md
            or "No markdown content generated",  # Display the markdown for the entire PDF
            combined_md_raw
            or "No markdown content generated",  # Display the raw markdown for the entire PDF
            (
                gr.update(value=download_zip_path, visible=True)
                if download_zip_path
                else gr.update(visible=False)
...
def create_gradio_interface():
    ...
    with gr.Blocks(theme=gr.themes.Soft(), css=css, title="dots.ocr") as demo:
